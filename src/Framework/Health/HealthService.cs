...
namespace Spitfire.Framework.Health
{
    /// <summary>
    ...
    public class HealthService
    {
        ...
        {
            var renderingsRootItem = this.db.GetItem(Constants.SpitfireRenderingsRoot);
            ...
            var viewRenderingFiles = Directory.GetFiles(viewsFolder, "*.cshtml", SearchOption.AllDirectories).Select(x => ("/views/" + x.Replace(viewsFolder, string.Empty).Replace("\\", "/")).ToLower()).ToList();
            ...
            foreach (var renderingItem in renderingsRootItem.Axes.GetDescendants())
            {
                switch (renderingItem.Template.Name)
                {
                    case "View rendering":
                        ...
                        {
                            this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Error, "Rendering's path not set", renderingItem));
                            continue;
                        }
                        ...
                        {
                            this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Error, "File not found: " + path, renderingItem));
                            continue;
                        }
                        ...
                        var expectedFile = renderingItem.Paths.Path.ToLower().Replace(ViewRootPath, string.Empty).Replace(" ", string.Empty);
                        ...
                        if (!string.Equals(path, expectedPath, StringComparison.InvariantCultureIgnoreCase))
                            this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Warning, "Expected path: " + expectedPath, renderingItem));

                        // Check caching enabled
                        if (renderingItem["Cacheable"] != "1")
                            this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Info, "No caching enabled", renderingItem));
                        ...
                        if (!string.IsNullOrEmpty(modelPath))
                        {
                            ...
                            if (modelItem == null)
                            {
                                this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Error, "ModelItem not found: " + modelPath, renderingItem));

                                continue;
                            }

                            var expectedModelPath = renderingItem.Paths.Path.ToLower().Replace(ViewRootPath, ModelRootPath);
                            ...
                            {
                                var message = string.Format("Expected Model path: {0} - Actual: {1}", expectedModelPath, modelPath);

                                this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Warning, message, renderingItem));
                            }
                            ...
                            if (modelType == null)
                                this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Error, "Type not found on model: " + modelTypeString, renderingItem));

                            // Check the namespaces match the structure of the content tree
                            var expectedNamespace = renderingItem.Paths.Path.ToLower().Replace(ViewRootPath, string.Empty).Replace("/", ".");
                            ...
                            {
                                var message = string.Format("Expected Model Type: {0} - Actual: {1}", expectedModelType, modelItem["Model Type"]);
                            ...
                            if (File.Exists(renderingLocation))
                            {
                                ...
                                if (!renderingContents.Contains(expectedModelDeclaration))
                                    this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Warning, "Model declaration missing: " + expectedModelDeclaration, renderingItem));

                                // Check Datasource Template/Location set
                                if (renderingItem["Datasource Location"] == string.Empty || renderingItem["Datasource Template"] == string.Empty)
                                    this.AddRenderingIssue(new HealthIssueRendering(HealthIssueSeverity.Warning, "Datasource Location or Datasource Template not set", renderingItem));
                            }
                        }
            ...
            {
                if (leftoverViewRendering.Contains("/shared/") || leftoverViewRendering.Contains("/layouts/") || leftoverViewRendering.Contains("/form/"))
                    continue;
