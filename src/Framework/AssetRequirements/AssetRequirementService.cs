namespace Spitfire.Modules.AssetRequirements
{
    ...
    public class AssetRequirementService
    {
        ...
        public static AssetRequirementService Current
        {
            get
            {
                const string Key = "Spitfire.Modules.AssetRequirementService";
                if (Context.Items[Key] == null)
                {
                    Context.Items[Key] = new AssetRequirementService();
                }

                return (AssetRequirementService)Context.Items[Key];
            }
        }
        ...
        public HtmlString RenderJavaScript()
        {
            ...
            foreach (var item in this.items.Where(x => x.Type == AssetType.JavaScript && x.File != null))
            {
                sb.AppendFormat("<script src=\"{0}\"></script>", item.File).AppendLine();
            }

            if (this.items.Any(x => x.Inline != null))
            {
                sb.AppendLine("<script>\njQuery2(document).ready(function() {");

                foreach (var item in this.items.Where(x => x.Type == AssetType.JavaScript && x.Inline != null))
                {
        ...
        {
            var sb = new StringBuilder();
            foreach (var item in this.items.Where(x => x.Type == AssetType.Css && x.File != null))
            {
            ...
            if (this.items.Any(x => x.Inline != null))
            {
                ...
                foreach (var item in this.items.Where(x => x.Type == AssetType.Css && x.Inline != null))
                {
                    sb.AppendLine(item.Inline);
                }
        ...
        {
            this.Add(new AssetRequirement(AssetType.JavaScript, null, inline, addOnceToken), preventAddToCache);
        }
        ...
        {
            this.Add(new AssetRequirement(AssetType.Css, null, inline, addOnceToken), preventAddToCache);
        }
    }
}